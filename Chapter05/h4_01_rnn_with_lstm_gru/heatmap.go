...
func Heatmap(a *tensor.Dense) (p *plot.Plot, H, W vg.Length, err error) {
	...
	case 2:
	default:
		return nil, vg.Length(0), vg.Length(0), errors.Errorf("Can't do a tensor with shape %v", a.Shape())
	}

	m, err := tensor.ToMat64(a, tensor.UseUnsafe())
	if err != nil {
		return nil, vg.Length(0), vg.Length(0), err
	}
	...
	sh := a.Shape()
	H = vg.Length(sh[0]) * vg.Centimeter
	W = vg.Length(sh[1]) * vg.Centimeter
	return p, H, W, nil
}
